"""inference.py — Standalone text-to-image inference for Chroma.

Builds the three Chroma modules (Flux VAE, T5-XXL text encoder, Chroma
transformer), loads the frozen base weights (native flow keys), optionally
injects a LoRA adapter, and generates images with the native
`chroma.model.sampling.sample` pipeline (Euler + classifier-free guidance,
resolution-aware shifted timesteps; the model's own guidance INPUT stays 0 —
Chroma1 convention).

The model is diced once into a flat chunk list (`chroma/model/chunks.py`),
...
-----
    ...
    uv run python chroma/inference.py \\
        --config chroma/configs/train_offload_lora.json \\
    ...
    uv run python chroma/inference.py --config ... --prompt "p1" --prompt "p2"
    uv run python chroma/inference.py --config ... --prompts-file prompts.txt

    # No LoRA (base model) — useful as a control:
    uv run python chroma/inference.py --config ... --no-lora --prompt "..."

    # Pipeline-parallel across several GPUs:
    uv run python chroma/inference.py --config ... --no-lora \\
    ...
    CUDA_VISIBLE_DEVICES=0 uv run python chroma/inference.py --config ... \\
        --offload --num-shards 4 --shard 0 --prompts-file ... --out-dir previews/x

Config fields used (all present in chroma/configs/train_*.json):
    chroma_config, encoder_config, vae_model_id, encoder_model_id,
    chroma_checkpoint, lora_checkpoint, lora_rank, lora_alpha,
...
# Allow running both as `python chroma/inference.py` and `python -m chroma.inference`.
...
from chroma.model.encoder import T5Conditioner
from chroma.model.lora import LoRALinear, inject_lora
...
from chroma.train_utils import _pin_sdpa_backends
...
def build_pipelines(dit, encoder_id, enc_cfg, devices, dtype=torch.bfloat16,
    ...
    print("[infer] Building Chroma pipeline ...")
...
def sample_pipeline(
    ...
    *,
    ...
    """Pipeline-parallel mirror of chroma.model.sampling.sample (same seeds -> same noise)."""
    from PIL import Image
    from einops import rearrange

    from chroma.model.chunks import set_dit_seq
    ...
    x1 = (minres // align) ** 2
    x2 = (maxres // align) ** 2
    ts = chroma_timesteps(x.shape[1], steps, x1, x2, y1=y1, y2=y2, mu=mu)

    set_dit_seq(dit_chunks, txt.shape[1], x.shape[1])
    guid = torch.zeros(n, dtype=torch.float32, device=device)
    ...
    for i, (tcurr, tprev) in enumerate(zip(ts[:-1], ts[1:])):
        with (trace.iteration(i) if trace else nullcontext()):
            ...
            with (trace.span("cond") if trace else nullcontext()):
                cond = dit_pipe.infer(
                    (img, img_ids, txt, txtmask, t, guid), n_microbatches=n
                ).to(device)
            if cfg:
                with (trace.span("uncond") if trace else nullcontext()):
                    uncond = dit_pipe.infer(
                        (img, img_ids, untxt, untxtmask, t, guid), n_microbatches=n
                    ).to(device)
    ...
    latent = vae_unflatten(
        img, height // ae.compression, width // ae.compression, patch
    )
    ...
    *,
    ...
    for i, (tcurr, tprev) in enumerate(zip(ts[:-1], ts[1:])):
        with (trace.iteration(i) if trace else nullcontext()):
            t = torch.full((n,), tcurr, dtype=torch.float32, device=device)
            with (trace.span("cond") if trace else nullcontext()):
                cond = model((img, img_ids, txt, txtmask, t, guid))
            if cfg:
                with (trace.span("uncond") if trace else nullcontext()):
                    uncond = model((img, img_ids, untxt, untxtmask, t, guid))
                v = uncond + guidance * (cond - uncond)
            else:
                v = cond
            img = img + (tprev - tcurr) * v.to(img.dtype)
    ...
    # Decode one sample at a time (same reasoning as sample_pipeline).
    outs = []
    for i in range(latent.shape[0]):
        px = ae.decode(latent[i : i + 1].to(torch.bfloat16))
        px = px.clamp(-1, 1) * 0.5 + 0.5
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Standalone Chroma text-to-image inference (with optional LoRA).",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    ap.add_argument("--config", default="chroma/configs/train_offload_lora.json",
    ...
    ap.add_argument("--guidance", type=float, default=None,
                    help="External CFG scale (default: config preview_cfg_scale, "
                         "fallback 4.0). The model's own guidance input stays 0.")
    ...
    ap.add_argument("--chroma-checkpoint", default=None,
                    help="Override config chroma_checkpoint (e.g. a merged full model).")
    ...
    ap.add_argument("--devices", nargs="+", default=None,
                    help="Pipeline stage devices, one per stage (default: all GPUs). "
                         "The first device is the driver (noise/VAE/decode) — list "
    ...
    ap.add_argument("--chunks-per-stage", type=int, nargs="+", default=None,
                    help="Chunks per pipeline stage (default: balanced by WEIGHT "
                         "BYTES). Must sum to 2 + ceil(19/bpc) + ceil(38/bpc).")
    ...
    ap.add_argument("--blocks-per-chunk", type=int, default=1,
                    help="Transformer blocks per chunk (default 1 -> 59 chunks).")
    ...
    dit_cfg_name = cfg.get("chroma_config", "chroma1")
    enc_cfg = ENCODER_CONFIGS[cfg.get("encoder_config", "t5_xxl")]
    encoder_id = cfg.get("encoder_model_id", enc_cfg.model_id)
    chroma_ckpt = args.chroma_checkpoint or cfg.get("chroma_checkpoint")
    ...
    # VAE + text encoder
    # ------------------------------------------------------------------
    print("[infer] Loading VAE (Flux) ...")
    ae = FluxAutoencoder(cfg.get("vae_model_id", enc_cfg.model_id))
    ae.ae = ae.ae.to(dtype).eval().requires_grad_(False)
    ae = ae.to(device).eval().requires_grad_(False)
    if args.offload:
        # Tiled decode keeps the 1024px upsample buffers to a few hundred MB.
        ae.ae.enable_tiling()
    ...
    # Chroma + LoRA
    # ------------------------------------------------------------------
    print(f"[infer] Building Chroma ({dit_cfg_name}) ...")
    dit_cfg = CHROMA_CONFIGS[dit_cfg_name]
    with torch.device("meta"):
        dit = Chroma(dit_cfg)
    if not chroma_ckpt:
        print("[err] config has no chroma_checkpoint; cannot run inference on random init.",
              file=sys.stderr)
        return 1
    print(f"[infer]   loading base weights: {chroma_ckpt}")
    if args.offload:
        ...
        with safe_open(chroma_ckpt, framework="pt", device="cpu") as f:
            for k in f.keys():
    ...
    if args.pipeline:
        ...
        from chroma.model.chunks import build_dit_chunks, chunk_bytes

        patch = dit.config.patch
        dit = dit.to(dtype).eval().requires_grad_(False)  # stays on CPU

        # Pre-encode every prompt of this shard, then free the ~9 GB encoder
        # so the streaming window + VAE decode fit in the remaining VRAM.
        ...
        # dtype — chroma.model.sampling.sample runs the denoise loop in bf16.
        ...
        with torch.autocast(device.type, dtype):
            if args.pipeline:
                images = sample_pipeline(
                    dit_pipe, dit_chunks, enc_pipe, tokenizer, ae, patch,
                    chunk,
                ...
                images = sample_offload(
                    offload_model, offload_chunks, ae, patch,
