...
Builds a TINY Chroma (~0.3M params) and compares, on forward output AND every
parameter gradient, the monolithic ``Chroma.forward`` against every way
RamTorch can execute ``build_dit_chunks()``:
  ...
  - ``OffloadModel(chunks)`` directly (the path chroma/inference.py uses)
...
mod-tensor pass-through, tuple relay, the static txt/img split).
...
Run:
    uv run python chroma/tools/check_chunk_parity.py
    uv run python -m chroma.tools.check_chunk_parity --blocks-per-chunk 2
"""
...
def make_inputs(cfg, seed: int = 1234):
    ...
    return (img, img_ids, txt, txt_mask, t, guidance), target, TXTLEN, img.shape[1]


def grads_of(dit: Chroma) -> dict[str, torch.Tensor]:
    return {
...
def main():
    ...
    print(
        f"Tiny Chroma: {sum(p.numel() for p in base.parameters())/1e6:.2f}M params, "
        f"{TINY.depth} double + {TINY.depth_single_blocks} single blocks -> "
        f"{n_chunks} chunks (blocks_per_chunk={bpc}), tol={args.tol:g}\n"
    )
