"""train.py — Chunk-based flow-matching fine-tuner for Chroma.

The ~8.9B Chroma transformer is diced ONCE into a flat list of chunk modules
(`chroma/model/chunks.py`: ``[embed, double x 19, single x 38, head]``) and
handed to RamTorch's `Pipeline`. Which hardware strategy you get is then just
a flag — there is one construction and one training loop for all three:
...
step's M. VAE calls and previews are bounded by its microbatch size.
...
Run:
    uv run python chroma/train.py chroma/configs/train_offload_lora.json
    uv run python chroma/train.py chroma/configs/train_pipeline_lora.json
    uv run python -m chroma.train chroma/configs/train_pipeline_offload_full.json
"""
...
# Allow running both as `python chroma/train.py` and `python -m chroma.train`.
...
from chroma.model.lora import inject_lora, lora_state_dict, trainable_param_count
...
from chroma.train_utils import (
    _mu_from_seq_len,
    _pin_sdpa_backends,
    sample_timesteps,
    vae_decode,
...
def preview(
    ...
    tensor [2*n, 3, H, W] in [-1, 1]: generated samples followed by decoded
    GT. The model's guidance INPUT stays 0 (Chroma1 convention); cfg_scale is
    classic external CFG against the empty prompt."""
    ...
    for tcurr, tprev in zip(ts[:-1], ts[1:]):
        ...
        uncond = dit_pipe.infer(
            (img, img_ids, untxt, untxtmask, t_vec, guid), n_microbatches=1
        ).to(device)
...
def train(cfg: dict, config_path: str):
    ...
    for dev in devices:
        ...
        # Sliced (per-sample) encode/decode — caps the preview decode's
        # driver-side fp32 GroupNorm/conv activations at ~4 GB instead of
        # ~15 GB at 1024px batch 4; enable_tiling() is a measured no-op for
        # this VAE/diffusers combo (see chroma/train_tdm.py and the
        # 2026-08-21 worklog entry).
        a.ae.enable_slicing()
        aes[dev] = a
    ...
    if mode == "lora":
        if chroma_ckpt:
            print(f"  Loading weights from {chroma_ckpt}...")
        ...
        if full_ckpt_cfg:
            ...
            if not chroma_ckpt:
                raise RuntimeError(
                    "mode='full' + lora_checkpoint requires chroma_checkpoint."
                )
            print(f"  Loading base weights from {chroma_ckpt}...")
            base_sd = load_file(chroma_ckpt, device="cpu")
    ...
    print(f"  Chroma [{mode} mode]: {trainable_n/1e6:.1f}M trainable / "
    ...
    dataset = ParquetTextImageDataset(
        ...
        **batching_kwargs,
        ...
        tokenizer=None,          # tokenization happens in ChromaCaptionTokenizer
        max_text_len=0,
    )
    ...
    # Benchmark runs set this false: a full-FT checkpoint is ~18 GB on disk.
    ...
    while True:
        ...
        for batch_data in pbar:
            ...
            x_t_tok, img_ids = prepare(x_t, patch)
            v_target, _ = prepare(x0_noise - x0_clean, patch)
            imglen = x_t_tok.shape[1]
            guid = torch.zeros(B, dtype=torch.float32, device=driver)
            ...
            if eval_interval > 0 and global_step % eval_interval == 0:
                dit.eval()
                rows = preview(
                    dit_pipe, dit_chunks, enc_pipe, tokenizer, aes[driver],
                    patch, compression,
...
if __name__ == "__main__":
    ...
    ap.add_argument("config", nargs="?",
                    default="chroma/configs/train_offload_lora.json")
