...
TDM (krea2/train_tdm.py) runs teacher, student and fake score off ONE frozen
weight buffer by switching LoRA adapters between forward passes. This check
builds a TINY SingleStreamDiT on CPU and verifies, in seconds:
...
Run:
    uv run python krea2/tools/check_tdm_roles.py
    uv run python -m krea2.tools.check_tdm_roles
"""
...
def main():
    ...
    check("role=None == un-injected base", d == 0.0, f"d={d:.3e}")
    ...
    # NOTE the shared non-LoRA trainables (norms/modulation) keep
