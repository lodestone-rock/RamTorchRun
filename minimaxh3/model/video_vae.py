...
#
# Ported from diffusers' `AutoencoderKLMiniMaxH3`
# (diffusers/models/autoencoders/autoencoder_kl_minimax_h3.py) to plain
# PyTorch: no diffusers imports, but the module tree and every operation are
# kept identical so the official checkpoint loads with `strict=True` and the
# numerics are bit-exact.

"""video_vae.py — MiniMax-H3 video VAE (causal 3D-CNN encoder + ViT decoder).

The encoder is a causal 3D CNN (temporal padding only on the left, GroupNorm
applied per frame in isolation). The decoder is a non-causal 36-layer ViT:
every latent voxel becomes one token, learned register tokens plus one zero
token are appended, and a patch projection expands each token back into a
`patch_size_t x patch_size x patch_size` pixel block.

Latents are normalized with per-channel `latents_mean` / `latents_std`
rather than a `scaling_factor`: a pipeline encodes with
`(latent - latents_mean) / latents_std` and decodes with
`latent * latents_std + latents_mean`. Pixels are ImageNet-normalized RGB
over a `[0, 1]` base range: `encode` expects `(pixel - imagenet_mean) /
imagenet_std` and `decode` returns values in that same space.
...
`17 * n + 5` pixel frames map to `5 * n + 2` latent frames. Spatial tiling
is ON by default, matching the released checkpoint's outputs.
"""
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        for resnet in self.resnets:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                hidden_states = torch.utils.checkpoint.checkpoint(resnet, hidden_states, use_reentrant=False)
            else:
...
class MiniMaxH3VideoAttention(nn.Module):
    ...
    def forward(
        ...
        # Equivalent to diffusers' default (native) attention backend: SDPA over (B, H, S, D).
...
class MiniMaxH3VideoSwiGLU(nn.Module):
    r"""Gated linear unit with a SiLU gate; inlined from `diffusers.models.activations.SwiGLU`."""
...
class MiniMaxH3VideoFeedForward(nn.Module):
    r"""SwiGLU feed-forward; inlined from `diffusers.models.attention.FeedForward` with `activation_fn="swiglu"`."""
        ...
        for block in self.transformer_blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                hidden_states = torch.utils.checkpoint.checkpoint(
                    block, hidden_states, rotary_emb, use_reentrant=False
                )
            else:
...
class DiagonalGaussianDistribution:
    r"""Inlined from `diffusers.models.autoencoders.vae.DiagonalGaussianDistribution`."""
...
class AutoencoderKLMiniMaxH3(nn.Module):
    ...
    Unlike most autoencoders, spatial tiling is **on by default**: MiniMax-H3 was released with tiling enabled for
    both encoding and decoding, and the released frames are the blended-tile ones, so disabling tiling changes the
    output. Use `enable_tiling` to change the tile geometry, `disable_tiling` to turn it off.
    """
    ...
    def __init__(
        ...
        self.latents_mean = tuple(latents_mean)
        self.latents_std = tuple(latents_std)
    ...
    @classmethod
    def from_pretrained(cls, folder: str, device: torch.device | str = "cpu") -> "AutoencoderKLMiniMaxH3":
        r"""Build the model from `folder/config.json` and load the diffusers-format safetensors with `strict=True`."""
        ...
        index_files = sorted(glob.glob(os.path.join(folder, "*.safetensors.index.json")))
        if index_files:
            with open(index_files[0]) as f:
                weight_map = json.load(f)["weight_map"]
            shard_names = sorted(set(weight_map.values()))
        else:
            shard_names = sorted(os.path.basename(p) for p in glob.glob(os.path.join(folder, "*.safetensors")))
        if not shard_names:
            raise FileNotFoundError(f"No safetensors weights found in {folder}")
        state_dict = {}
        for shard_name in shard_names:
            state_dict.update(load_file(os.path.join(folder, shard_name)))
    ...
    def encode(self, x: torch.Tensor) -> DiagonalGaussianDistribution:
        ...
        """
        # The verified recipe keeps the weights in float32 (a pipeline runs decode under fp16 *autocast*), so a
        # pipeline holding lower-precision pixels must align them with the weights.
