...
class T5Conditioner(torch.nn.Module):
    ...
    Radiance conditions on the raw T5 encoder output, exactly as Chroma does —
    same encoder, same weights. Prompts are padded to a FIXED ``max_length``
    (flow convention), so the text length is static per run, which is what lets
    the chunked trainers fix ``txtlen`` once per step.
    """
