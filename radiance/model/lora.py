"""lora.py — Minimal LoRA injection for Radiance.
...
modulation tensors, conv biases — are left with requires_grad=True so
they fully update alongside the LoRA adapters.

Usage:
    inject_lora(radiance, rank=32, alpha=32.0)
    trainable = [p for p in radiance.parameters() if p.requires_grad]
    optimizer = AdamW(trainable, lr=1e-4)

Role-based multi-adapter mode (TDM distillation):

    inject_lora(radiance, rank=32, alpha=32.0, extra_roles=("fake",))
    set_lora_role(radiance, "default")  # student adapter (lora_A / lora_B)
    set_lora_role(radiance, "fake")     # fake-score adapter (lora_A_fake / ...)
    set_lora_role(radiance, None)       # frozen base only (the teacher)
