...
fake score, trainable student. Here all three share ONE frozen 9.5B weight
buffer: two LoRA adapters ride on the same base (`radiance/model/lora.py`
...
convention: x_t = (1-t) x0 + t eps, model predicts v = eps - x0, t: 1->0):

  1. Rollout (no grad, role=student): K Euler steps from noise on the shifted
     schedule, CFG-free, saving each step's input x_{t_m} and its x0 / eps
     predictions.
...
Everything above is chroma's TDM verbatim. What pixel space changes:

  - **No VAE.** The rollout runs at PIXEL resolution on ``[B, 3, H, W]``, and
    previews put the student's samples next to the dataloader's images with no
    decode. Images are only ever used for previews — the distillation itself
    needs captions alone.
  - **``pack_targets`` concatenates on the CHANNEL dim** (``[B, 8, H, W]`` =
    ``[x0_tgt(3) | x_in(3) | t(1) | w(1)]``), because the output is 4-D.
    RamTorch chunks ``targets`` along dim 0 only, so per-sample extras have to
    ride somewhere that survives that split.
  - **``set_x0_eps(0.0)``.** Every step here reconstructs ``x0 = x - t*v``, and
    that identity only holds when the model's residual divides by exactly ``t``.
    TDM's ``t`` never reaches 0 (the rollout uses ``ts[0..K-1]`` and
    ``draw_segment`` clamps tau to ``tdm_tau_min``), so no epsilon is needed.

Execution strategy is the same flag as `radiance/train.py`: ``parallelism`` in
{"offload", "pipeline", "pipeline-offload"} drives one Pipeline construction.

Run:
    uv run python radiance/train_tdm.py radiance/configs/train_tdm_lora.json
    uv run python -m radiance.train_tdm radiance/configs/train_tdm_smoke.json --parallelism pipeline
"""
...
# Allow running both as `python radiance/train_tdm.py` and `python -m radiance.train_tdm`.
...
def pack_targets(
    x0_tgt: torch.Tensor,    # [B, C, H, W] x0-space regression target
    x_in: torch.Tensor,      # [B, C, H, W] the step's noisy input (to rebuild x0)
    t: torch.Tensor,         # [B] the step's timestep
    w: torch.Tensor,         # [B] per-sample loss weight
) -> torch.Tensor:
    B, _, H, W = x0_tgt.shape
    t_col = t.float().view(B, 1, 1, 1).expand(B, 1, H, W)
    w_col = w.float().view(B, 1, 1, 1).expand(B, 1, H, W)
...
def make_tdm_loss(huber_c: float | None):
    ...
    def loss_fn(out: torch.Tensor, tgt: torch.Tensor) -> torch.Tensor:
        C = out.shape[1]
        x0_tgt = tgt[:, :C].float()
        x_in = tgt[:, C:2 * C].float()
        t = tgt[:, 2 * C:2 * C + 1, :1, :1]          # [B, 1, 1, 1]
        w = tgt[:, 2 * C + 1, 0, 0]                  # [B]
        x0 = x_in - t * out.float()
        diff = x0 - x0_tgt
        if huber_c is not None:
            ...
            el = diff * diff
        return (el.mean(dim=(1, 2, 3)) * w).mean()

    return loss_fn


# ---------------------------------------------------------------------------
# Preview — K-step CFG-free student sampling next to the GT images
# ---------------------------------------------------------------------------
...
def preview_tdm(
    ...
    float32 CPU tensor [2*n, 3, H, W] in [-1, 1]: samples then the GT images."""
...
def train(cfg: dict, config_path: str):
    ...
    # near zero (early training / matched distributions).
    #
    # !! WARNING: DO NOT USE the ratio rescale. Three chroma runs died with
    # it (2026-08-21 and the halflr-decoupled run retired 2026-08-23): the
    # critic outruns the student, loss_d climbs to 0.03-0.25 and spikes,
    # where the legacy-math run on the same setup sits at 0.02-0.04 and
    # stays healthy. Keep tdm_ca_ratio = null until it is rediagnosed.
    ...
    # ------------------------------------------------------------------
    # Radiance — teacher weights + two LoRA roles (student=default, fake)
    # ------------------------------------------------------------------
    print("Loading Radiance (teacher base + student/fake LoRA roles)...")
    ...
    if not radiance_ckpt:
        raise RuntimeError(
            "TDM distills a pretrained teacher — radiance_checkpoint is required."
        )
    print(f"  Loading teacher weights from {radiance_ckpt}...")
    ...
    # trainables inject_lora leaves requires_grad=True (RMSNorm scales, the
    # patchify conv, the head conv, and every excluded-prefix Linear — which
    # for radiance means the whole Approximator and the NeRF embedder) are
    ...
    print(f"  Radiance [tdm mode]: student {n_student/1e6:.1f}M + fake "
    ...
    # Every phase here reconstructs x0 = x - t*v, so the residual must divide
    # by exactly t (see the module docstring).
    ...
    # Per-stage, not global: the NeRF chunks relay a shorter tuple.
    ...
    def dit_infer(x, t, txt_mbs, txtmask_mbs, img_ids):
        ...
        if n_mb == 1:
            out = dit_pipe.infer(
                (x, img_ids, txt_mbs[0], txtmask_mbs[0], t),
                n_microbatches=1,
            )
    ...
    def draw_segment(ts_t, xs, x0s, epss, B):
        ...
        Returns (m, t_m, x_tm, x0_hat, tau, x_tau, eps_mix, xi):
          ...
          x_tm    [B,C,H,W]  the student step's noisy input at t_m
          x0_hat  [B,C,H,W]  the student's clean prediction at that step
          tau     [B]        diffused timestep for the score evaluations
          x_tau   [B,C,H,W]  the ODE point at the segment end, renoised to tau
          eps_mix [B,C,H,W]  effective total noise in x_tau w.r.t. x0_hat
          xi      [B,C,H,W]  the fresh noise used for the renoising
        """
        ...
        gather_idx = m.view(1, B, 1, 1, 1).expand(1, B, C, H, W)
    ...
    while True:
        ...
        for batch_data in pbar:
            ...
            # The rollout is pure noise -> image; the dataloader's images only
            # set the shape here (and feed the preview's GT column).
            height, width = images.shape[2], images.shape[3]
            img_tok = torch.randn(
                B, dit_cfg.in_channels, height, width, device=driver, dtype=dtype
            )
            ...
            with torch.no_grad():
                ...
                is_w = torch.exp(
                    -0.5 * eps_mix.square().mean(dim=(1, 2, 3))
                    + 0.5 * xi.square().mean(dim=(1, 2, 3))
                )
            ...
            with torch.no_grad():
                ...
                if tdm_ca_ratio is None:
                    ...
                    # !! WARNING: unstable in practice — do not use (see the
                    # knob comment above / worklog 2026-08-21 and 2026-08-23).
                    dm = x0_real - x0_fake
                    ca = x0_real - x0_unc
                    n_dm = dm.abs().mean(dim=(1, 2, 3), keepdim=True)
                    n_ca = ca.abs().mean(dim=(1, 2, 3), keepdim=True).clamp(min=1e-6)
                    dm_hat = dm / torch.maximum(n_dm, tdm_dm_floor * n_ca)
                    ca_hat = ca / n_ca
                    u_mix = (1.0 - tdm_ca_ratio) * dm_hat + tdm_ca_ratio * ca_hat
                    magn = (
                        (x0_real_cfg - x0_fake)
                        .abs().mean(dim=(1, 2, 3), keepdim=True)
                    )
                    coop = x0_hat + magn * u_mix

                weighting = (
                    (x0_hat - x0_real_cfg).abs().mean(dim=(1, 2, 3)).clamp(min=1e-6)
                )
...
if __name__ == "__main__":
    ...
    ap.add_argument("config", nargs="?",
                    default="radiance/configs/train_tdm_lora.json")
